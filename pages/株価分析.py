...
with st.form(key='profile_form'):
    st.subheader('日系平均を表示')
    st.text("表示するデータの範囲を決める")
    start = st.text_input('始まり(0000-00-00(年-月-日))', key='start_input_1')
    end = st.text_input('終わり(0000-00-00(年-月-日))', key='end_input_1')
    name = st.selectbox('表示させたい値', ('Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume'), key='value_selector_1')
    bunseki = st.selectbox('分析名', ('ゴールデンクロスの検出', 'ARIMA', 'ランダムフォレストモデル'))
        ...
        if len(start) == 0 or len(end) == 0:
            ...
            if name is not None:
                ...
                if bunseki == 'ゴールデンクロスの検出':
                    ...
                    plt.title('日経平均株価　Golden Crosses')
                    plt.legend()
                    st.pyplot(plt)
                    ...
                    forecast_index = pd.date_range(start=date[-1], periods=steps + 1, freq='B')[1:]
